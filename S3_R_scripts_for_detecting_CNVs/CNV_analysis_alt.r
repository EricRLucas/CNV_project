...
# Check the input chromosome name makes sense
acceptable.chromosomes <- c('2L', '2R', '3L', '3R', 'X')
chrom <- commandArgs(trailingOnly=T)[1]
...
duplications.list.indices <- lapply(counts.list, find.full.dup.sequences, n = min.dup.length, threshold = threshold.copy.number)
...
for (this.sample in names(goodvar.duplications.list.indices)){
	...
	for (k in 1:nrow(this.cnv.table)){
		...
		this.lik.ratio <- lik.ratio(this.sample, this.raw.cnv)
...
# the start points of all the samples that have that CNV. 
